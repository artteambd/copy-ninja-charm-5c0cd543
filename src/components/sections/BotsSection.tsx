...
import { Sparkles, ArrowRight, Star, Eye, ShoppingCart, TrendingUp, Zap, Shield, ChevronDown, Grid3X3, LayoutList, Flame, Check, Clock, Users, Award, Bot, BarChart3 } from "lucide-react";
...
const FeaturedCard = ({ bot }: { bot: BotData }) => {
  ...
  return (
    ...
    >
      ...
      <div className={`absolute -inset-[2px] rounded-3xl bg-gradient-to-r ${bot.accent} opacity-40 group-hover:opacity-80 transition-opacity duration-700 blur-md`} />
      <motion.div
        className={`absolute -inset-[1px] rounded-3xl bg-gradient-to-r ${bot.accent}`}
        animate={{ opacity: [0.3, 0.6, 0.3] }}
        transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
      />

      <div className="relative rounded-3xl bg-card/95 backdrop-blur-xl border border-border/10 overflow-hidden">
            ...
            <div className="absolute top-5 left-5 flex flex-col gap-2">
              {bot.tag && (
                <motion.span initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.3 }}
                  className="font-orbitron text-[8px] tracking-[0.3em] px-4 py-2 rounded-full bg-background/70 backdrop-blur-md text-foreground border border-border/20 inline-flex items-center gap-1.5 w-fit">
                  <Flame size={10} className="text-amber-400" />{bot.tag}
                </motion.span>
              )}
              <motion.span initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.5 }}
                className="font-orbitron text-[8px] tracking-wider px-3 py-2 rounded-full bg-emerald-500/15 backdrop-blur-md text-emerald-400 border border-emerald-500/20 inline-flex items-center gap-1 w-fit">
                <Award size={9} /> #1 BEST SELLER
              </motion.span>
            </div>

            {/* Live viewers */}
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.7 }}
              className="absolute bottom-4 left-4 flex items-center gap-2 px-3 py-1.5 rounded-full bg-background/60 backdrop-blur-md border border-border/20">
              <span className="w-1.5 h-1.5 rounded-full bg-red-400 animate-pulse" />
              <span className="text-[8px] font-orbitron tracking-wider text-muted-foreground"><Eye size={9} className="inline mr-1" />67 VIEWING NOW</span>
            </motion.div>
          </div>
          ...
          <div className="lg:w-1/2 p-8 lg:p-10 flex flex-col justify-center">
            <div className="flex items-center gap-3 mb-4">
              <motion.div
                ...
                className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${bot.accent} flex items-center justify-center border border-border/10 shadow-lg`}>
                <Icon size={24} className="text-foreground" />
              </motion.div>
              <div>
                <h3 className="font-orbitron text-xl tracking-[0.15em] text-foreground">{bot.name}</h3>
                <div className="flex items-center gap-1.5 mt-1">
                  {Array.from({ length: 5 }).map((_, i) => <Star key={i} size={12} className="fill-amber-400 text-amber-400" />)}
                  <span className="text-[10px] text-muted-foreground ml-1">(256 reviews)</span>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-4 gap-3 mb-6">
              {bot.results.map((r) => (
                <motion.div key={r.label} whileHover={{ scale: 1.05 }} className="relative rounded-xl overflow-hidden text-center py-3 cursor-default">
                  <div className={`absolute inset-0 bg-gradient-to-b ${bot.accent} opacity-20`} />
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              {bot.features.map((f) => (
                <span key={f} className="inline-flex items-center gap-1.5 text-[9px] font-orbitron tracking-wider px-3 py-1.5 rounded-full bg-secondary/30 text-muted-foreground border border-border/10">
                  <Check size={8} className="text-emerald-400" />{f}
                </span>
              ))}
            </div>
            ...
            <div className="flex items-center gap-5">
              <div>
                ...
                <span className="text-[9px] font-orbitron tracking-wider text-emerald-400">SAVE ${Math.round(bot.price * 0.5)} · LIMITED</span>
              </div>
              <button onClick={() => navigate(`/bot/${bot.slug}`)} data-interactive
                ...
                <div className="absolute inset-[1px] rounded-[15px] bg-card/80 group-hover/btn:bg-card/40 transition-colors duration-300" />
      ...
      >
        <div className="relative rounded-2xl overflow-hidden">
          ...
          <div className="relative rounded-2xl bg-card/80 backdrop-blur-xl border border-border/10 p-5 flex flex-col md:flex-row gap-6">
            <div className="relative w-full md:w-64 h-44 md:h-auto rounded-xl overflow-hidden shrink-0">
              ...
              {bot.tag && (
                <span className="absolute top-3 left-3 font-orbitron text-[7px] tracking-[0.3em] px-2.5 py-1 rounded-full bg-background/70 backdrop-blur-md text-foreground border border-border/20">
                  <Sparkles size={8} className="inline mr-1 -mt-0.5" />{bot.tag}
                </span>
              )}
              <div className="absolute bottom-3 left-3 flex items-center gap-1.5 px-2 py-1 rounded-full bg-background/60 backdrop-blur-md">
            ...
            <div className="flex-1 flex flex-col justify-between min-w-0">
              <div>
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    <h3 className="font-orbitron text-sm tracking-[0.15em] text-foreground">{bot.name}</h3>
                    <div className="flex items-center gap-1.5 mt-0.5">
                      {Array.from({ length: 5 }).map((_, i) => <Star key={i} size={9} className="fill-amber-400 text-amber-400" />)}
                      <span className="text-[9px] text-muted-foreground ml-1">4.9 (128)</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-2 mb-3">
                  {bot.results.map((r) => (
                    <span key={r.label} className="text-[9px] font-orbitron tracking-wider px-3 py-1.5 rounded-lg bg-secondary/30 border border-border/10 text-foreground">
                ...
                <div className="flex flex-wrap gap-1.5 mb-3">
                  {bot.features.map((f) => (
                    <span key={f} className="inline-flex items-center gap-1 text-[8px] font-orbitron tracking-wider px-2 py-1 rounded-full bg-secondary/20 text-muted-foreground border border-border/10">
              ...
              <div className="flex items-center gap-4">
                <div>
                  <div className="flex items-baseline gap-1.5">
                    <span className="font-orbitron text-2xl text-foreground">${bot.price}</span>
                    <span className="text-[10px] text-muted-foreground line-through">${Math.round(bot.price * 1.5)}</span>
                  </div>
                  <span className="text-[8px] font-orbitron tracking-wider text-emerald-400">-33% OFF</span>
                </div>
                <button onClick={() => navigate(`/bot/${bot.slug}`)} data-interactive
                  className="ml-auto flex items-center gap-2 px-6 py-3 rounded-xl font-orbitron text-[9px] tracking-[0.2em] glass border border-border/20 hover:border-border/40 text-foreground transition-all group/btn">
  ...
  return (
    ...
    >
      ...
      >
        ...
        <motion.div
          className={`absolute -inset-[1px] rounded-3xl bg-gradient-to-br ${bot.accent}`}
          animate={hovered ? { opacity: [0.5, 1, 0.5] } : { opacity: 0 }}
          transition={{ duration: 2, repeat: Infinity }}
        />

        <div className="relative rounded-3xl bg-card/85 backdrop-blur-xl border border-border/10 overflow-hidden">
          {/* Image */}
          <div className="relative h-56 overflow-hidden">
            ...
            <div className="absolute top-3 left-3 flex flex-col gap-1.5">
              {bot.tag && (
                <motion.span initial={{ opacity: 0, x: -10 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}
                  className="font-orbitron text-[7px] tracking-[0.3em] px-3 py-1.5 rounded-full bg-background/60 backdrop-blur-md text-foreground border border-border/20 inline-flex items-center gap-1 w-fit">
                  <Flame size={9} className="text-amber-400" />{bot.tag}
                </motion.span>
              )}
            ...
            <div className="absolute bottom-3 right-4 flex items-center gap-2">
              ...
              <span className="flex items-center gap-1 text-[9px] text-muted-foreground"><Eye size={10} /> 47</span>
            </div>
          </div>
          ...
          <div className="p-5 pt-4">
            <div className="flex items-center gap-3 mb-3">
              <motion.div
                ...
                className={`w-11 h-11 rounded-xl bg-gradient-to-br ${bot.accent} flex items-center justify-center border border-border/10 group-hover:shadow-lg transition-shadow duration-500`}>
                <Icon size={18} className="text-foreground" />
              </motion.div>
            ...
            <div className="grid grid-cols-2 gap-2 mb-3">
              {bot.results.slice(0, 2).map((r) => (
                <motion.div key={r.label} whileHover={{ scale: 1.03 }} className="relative rounded-xl overflow-hidden text-center py-3 cursor-default">
            ...
            >
              ...
              <div className="absolute inset-[1px] rounded-[11px] bg-card/90 group-hover/btn:bg-card/40 transition-colors duration-300" />
              <span className="relative z-10 flex items-center justify-center gap-2">
    ...
    <section className="min-h-screen pt-32 pb-24 px-6 relative">
      ...
      <div className="max-w-6xl mx-auto text-center mb-14 relative z-10">
        ...
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}
          className="text-muted-foreground text-sm tracking-wider max-w-lg mx-auto leading-relaxed mb-8">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}
          ...
          {categories.map(({ label, icon: I }) => (
            <button key={label} data-interactive onClick={() => setActiveCategory(label)}
              className={`inline-flex items-center gap-2 px-4 py-2 rounded-full font-orbitron text-[9px] tracking-[0.2em] transition-all duration-300 border ${
                activeCategory === label
                  ? "bg-foreground/10 text-foreground border-border/40"
                  : "glass text-muted-foreground border-border/10 hover:text-foreground hover:border-border/30"
              }`}>
              <I size={11} /> {label}
            </button>
          ))}
